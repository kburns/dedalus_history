...
my_ncc = domain.new_field()
my_ncc.meta['x', 'y']['constant'] = True
my_ncc['g'] = 1
...
problem.parameters['this_ncc'] = my_ncc

problem.add_equation("(dx(u) + dy(v) + wz) = 0")
problem.add_equation("this_ncc*(dt(b) - P*(dx(dx(b)) + dy(dy(b)) + dz(bz))             ) = - u*dx(b) - v*dy(b) - w*bz")
problem.add_equation("this_ncc*(dt(u) - R*(dx(dx(u)) + dy(dy(u)) + dz(uz)) + dx(p)     ) = - u*dx(u) - v*dy(u) - w*uz")
problem.add_equation("this_ncc*(dt(v) - R*(dx(dx(v)) + dy(dy(v)) + dz(vz)) + dy(p)     ) = - u*dx(v) - v*dy(v) - w*vz")
problem.add_equation("this_ncc*(dt(w) - R*(dx(dx(w)) + dy(dy(w)) + dz(wz)) + dz(p) - b ) = - u*dx(w) - v*dy(w) - w*wz")
